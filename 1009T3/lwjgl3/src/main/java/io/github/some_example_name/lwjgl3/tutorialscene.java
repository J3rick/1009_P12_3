...
public class tutorialscene extends scene{
    ...
    public tutorialscene(String name, Texture bgImg, Color bgColor, OrthographicCamera camera, List<entity> entityList_in){
		super(name,bgColor,bgImg,camera,entityList_in);
    }

    public tutorialscene(String name, Texture bgImg_in, Color bgColor, OrthographicCamera camera){
		super(name,bgColor,bgImg_in,camera);
    }
    
    public tutorialscene(String name, Texture bgImg_in, Color bgColor, OrthographicCamera camera, scenetransitionmanager sceneTransitionManager, String skinPath){
		super(name,bgColor,bgImg_in,camera);
		bgImg = bgImg_in;
		this.font = new BitmapFont();
		font.setColor(Color.BLACK);
		this.sceneTransitionManager = sceneTransitionManager; // allows us to transition btwn scene from this scene
		this.tutorialSkin = new Skin(Gdx.files.internal(skinPath));
    }
    ...
    @Override
    public void init() {
    	tutorialStage = new Stage(new ScreenViewport());
    	...
    	//Begin tutorial layout
        buttonPlay = new TextButton("Play", tutorialSkin);
        tutorialTable.add(buttonPlay).expandY().bottom().padBottom(10);
        buttonPlay.addListener(new ChangeListener() {
        	@Override
        	public void changed(ChangeEvent event, Actor actor) {
        		System.out.println("Tutorial: Play button pressed");
        		loadScene("main");
        	}
        });
    }
